...
/**
 * Upserts documents in the database without returning them.
 *
 * ####Examples:
 *
 *     MyModel.upsert({ age: { $gt: 18 } }, { oldEnough: true }, fn);
 *     MyModel.upsert({ name: 'Tobi' }, { ferret: true }, { multi: true }, function (err, numberAffected, raw) {
 *       if (err) return handleError(err);
 *       console.log('The number of upserted documents was %d', numberAffected);
 *       console.log('The raw response from Mongo was ', raw);
 *     });
 *
 * ####Valid options:
 *
 *  - `safe` (boolean) safe mode (defaults to value set in schema (true))
 *  - `multi` (boolean) whether multiple documents should be upserted (false)
 *  - `strict` (boolean) overrides the `strict` option for this upsert
 *  - `overwrite` (boolean) disables upsert-only mode, allowing you to overwrite the doc (false)
 *
 * All values are cast to their appropriate SchemaTypes before being sent.
 *
 * The `callback` function receives `(err, numberAffected, rawResponse)`.
 *
 * - `err` is the error if any occurred
 * - `numberAffected` is the count of upserted documents Mongo reported
 * - `rawResponse` is the full response from Mongo
 *
 * ####Note:
 *
 ...
 * ####Example:
 *
 *     var query = { unique: '!007' };
 *     Model.upsert(query, { name: 'jason borne', unique: '!007' }, options, callback)
 *
 *     // is sent as
 *     Model.update(query, { $set: { name: 'jason borne', unique: '!007' }}, options, callback)
 *     // options.upsert is set to true in the 'upsert' helper method
 *     // If overwrite is true, sent *without* the $set wrapper.
 *
 * This helps prevent accidentally overwriting an entire document, and instead merges the data using something akin to 'Object.merge'.
 *
 * ####Note:
 *
 * Be careful to not use an existing model instance for the upsert clause (this won't work and can cause weird behavior). Also, ensure that the upsert clause does not have an _id property, which causes Mongo to return a "Mod on _id not allowed" error.
 *
 * ####Note:
 *
 * To upsert documents without waiting for a response from MongoDB, do not pass a `callback`, then call `exec` on the returned [Query](#query-js):
 *
 *     Comment.upsert({ unique: '!007' }, { $set: { text: 'changed' }}).exec();
 *
 * @see strict http://mongoosejs.com/docs/guide.html#strict
 * @param {Object} conditions
 * @param {Object} doc
...
Model.upsert = function upsert (conditions, doc, options, callback) {
  var mq = new Query({}, {}, this, this.collection);
  // gh-2406
  // make local deep copy of conditions
  conditions = utils.clone(conditions);
