 ...
 * ####Available options
 *
 * - `new`: bool - false to return the modified document rather than the original. defaults to false (changed in 4.0)
...
Query.prototype.update = function (conditions, doc, options, callback) {
 ...
 /**
 * 
 * Wrapper method around `update(criteria, doc, {upsert: true}, callback)`
 *
 * ####Example
 *     var q = Model.where({ _id: id });
 * 
 *     // Most common usage:
 *     q.upsert({ $set: { name: 'bob' }}, callback); // shorthand for q.update(criteria, {upsert: true}, callback)
 *
 *     // Chainable patterns:
 *     q.update({ $set: { name: 'bob' }})
 *       .upsert(callback);         // executed with upsert default set to true
 *
 *     q.update({ $set: { name: 'bob' }}, { upsert: true })
 *       .upsert(false, callback);  // executed with upsert 'switched off' aka false
 *
 * Based on the Query.update method.
 *
 * @param {Object} [criteria]
 * @param {Object} [doc] the update command
 * @param {Object} [options]
 * @param {Function} [callback]
 * @return {Query} this
 * @see Model.upsert #model_Model.upsert
 * @see update http://docs.mongodb.org/manual/reference/method/db.collection.update/
 * @api public
 */
Query.prototype.upsert = function (conditions, doc, options, callback) {
  var castedDoc, castedQuery, oldCb;
  if ('function' === typeof options) {
    // Scenario: update(conditions, doc, callback)
    callback = options;
    options = null;
  ...
  if ('boolean' === typeof options) {
    // Scenario: upsert(conditions, doc, Boolean, [callback])
    options = { upsert: options };
  } else if ('boolean' === typeof conditions) {
    // Scenario: upsert(Boolean, [callback])
    options = { upsert: conditions };
  } else {
    // override the upsert option 
    if (options) {
      options.upsert = true;
    } else {
      options = { upsert: true };
  ...
  var result = Query.prototype.update.apply(this, arguments);

  console.log('\nQUERY.UPDATE', Object.keys(result).sort().join(', '), '\nQUERY.UPDATE', Object.keys(this).sort().join(', '), '\n');
  ...
  // oldCb = callback;
  // if (oldCb) {
  //   callback = function __cb(error, result) {
  //     oldCb(error, result ? result.result : { ok: 0, n: 0, nModified: 0 });
  //   };
  // }
